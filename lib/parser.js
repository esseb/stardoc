...
/**
 * Parse style comments in the given style file and return an array of style
 * objects.
 *
...
function parsedComment(comment) {
  ...
  if (description) {
    ...
    markdownDescription = markdown.toHTML(description);
  }
  ...
  // Extract category and name from the id parameter.
  if (!parameters.id) {
    return;
  }

  var idComponents = parameters.id.split('/');
  if (idComponents.length != 2) {
    return;
  }

  var category = idComponents[0];
  var name = idComponents[1];
